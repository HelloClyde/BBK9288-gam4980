...
本工程另提供独立于播放器的 **GAM → 9288 EXE 原生编译界面**：选择游戏、
修改程序名称、预览或替换四灰阶图标，一键编译为安装在“娱乐”分类的独立程序。
本机启动入口为 `build/GAM-to-9288-Converter.exe`，或运行 `python GAM-to-9288.pyw`。
启动器依赖当前工程、Python、9288 SDK 和修正版 S1C33 工具链，不是便携安装包。

详见 [转换器使用说明](docs/gam-converter-gui.md)。编译成功不代表所有游戏功能已经验证；
生成物包含游戏资源，不随源码或播放器 release 自动发布。
...
- 9288 的 320×240 屏幕上，将 159×96 游戏 LCD 精确放大两倍为 318×192，
  显示在 `(1, 24)`，不裁切、不插值。
- 绘图与文字采用原生 packed 合成，在完整绘图边界批量提交 2× 画面，
  避免逐 tile 展示和持久白线；GUI 兼容路径保留 SDK 输出。
- 实验性 ABI 5 NAT 包按函数加载11个图形与2个字形计算入口，相关算法不再
  重复链接进该版 EXE；详见[动态 NAT 绘图与文字](docs/native-graphics-framebuffer.md)。
...
- 提供实验性的游戏专用 PC 编译器：它把受容量约束的 GAM 初始工作集编译成
  S1C33 ELF 并直接链接进 KF2，同时保留 IRAM 安全回退；不会再生成约 4.7 MiB
  的完整分页代码。第一版《伏魔记》专用 KF2 为 984,577 字节，仍从原始 GAM
  读取资源和未覆盖代码。用法与边界见
  [`docs/gam-native-compiler.md`](docs/gam-native-compiler.md)。
