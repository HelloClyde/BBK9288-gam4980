...
- 选择游戏后显示分阶段 Loading 文本，明确区分运行库准备、文件读取、HLE
  匹配、CFG 分析、AOT 索引、存档读取和游戏启动；AOT 阶段显示 pass 进度。
...
- 正式构建默认启用受保护的 E.BIN 离线 AOT：从启动到开场剧情的 8000 帧
  完整轨迹中，共发现 926 个“物理 PC + 虚拟 PC”固件基本块，现已全部
  预先生成为 S1C33 C 超块。`8.BIN` 是字库/数据 ROM，同一轨迹没有任何
  6502 指令从中取出，因此不做伪代码翻译。原 47 块的安全边保留直接衔接，
  新增块在边界返回调度器检查中断、Timer 和 bank；每个块首次进入仍校验
  物理映射和 ROM 签名，不匹配便自动回退解释器。当前 KF2 约 803.2 KiB，
  `.bss` 约 10.5 KiB、`.scratch` 约 388.3 KiB，不使用 JIT 缓冲区或额外堆内存，也不绑定、
  不打包任何 `.gam`。详见 [`docs/aot-profiling.md`](docs/aot-profiling.md)。
- 游戏加载时 AOT 已升级为 C6502 编译器感知的整程序翻译。离线生成器从旧 A 系列
  ...
  回退。它不生成可执行 RAM，也不绑定或打包某个游戏。三款 GAM 的语义 AOT
