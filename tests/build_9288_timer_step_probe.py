#!/usr/bin/env python3
"""Build the minimal BBK 9288 GUI timer step probe."""
...
BUILD = ROOT / "build" / "timer-step-probe"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", type=Path, default=BUILD / "TIMER_STEP.exe"
    )
...
def main() -> None:
    ...
    sources = [
        ...
        ROOT / "tests" / "9288_timer_step_probe.c",
    ]
    objects: list[Path] = []
    ...
    elf = BUILD / "TIMER_STEP.elf"
    map_path = BUILD / "TIMER_STEP.map"
    run(
        [
            clang,
            "--target=s1c33-none-elf",
    ...
    app[12:28] = b"TIMER_STEP".ljust(16, b"\0")
