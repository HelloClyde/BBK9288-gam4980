...
#define S6502_TEST_STOP_PC 0x9001u
#include "../src/gam4980_core.c"

#define RANDOM_CASES 5000u

static uint32_t random_state = 0x859e8659u;
...
int main(int argc, char **argv)
{
    ...
    for (case_id = 0u; case_id < RANDOM_CASES; ++case_id) {
        ...
        s6502_t actual_cpu;
        uint16_t normal = (uint16_t)(0x0800u + next_random() % 0x0800u);
        uint16_t left = (uint16_t)(0x1000u + next_random() % 0x0800u);
        uint16_t alternate = (uint16_t)(0x1800u + next_random() % 0x0600u);
        ...
        sys.ram[_SYSCON] = 0u;
        sys.ram[0x2080u] = (uint8_t)(next_random() & 1u);
        sys.ram[0x2081u] = (uint8_t)next_random();
        sys.ram[0x3au] = (uint8_t)normal;
        sys.ram[0x3bu] = (uint8_t)(normal >> 8);
        sys.ram[0x38u] = (uint8_t)left;
        sys.ram[0x39u] = (uint8_t)(left >> 8);
        s6502_page3[0xe5u] = (uint8_t)(next_random() % 3u);
        if ((next_random() & 3u) == 0u) {
            s6502_page3[0xe6u] = (uint8_t)normal;
            s6502_page3[0xe7u] = (uint8_t)(normal >> 8);
        } else {
            s6502_page3[0xe6u] = (uint8_t)next_random();
            s6502_page3[0xe7u] = (uint8_t)next_random();
        }
        s6502_page3[0xe8u] = (uint8_t)alternate;
        s6502_page3[0xe9u] = (uint8_t)(alternate >> 8);
        initial_cpu.pc = 0x859eu;
        initial_cpu.ac = (uint8_t)next_random();
        initial_cpu.ix = (uint8_t)next_random();
        initial_cpu.iy = (uint8_t)next_random();
        initial_cpu.sp = (uint8_t)(0x20u + next_random() % 0xb0u);
        initial_cpu.status = (uint8_t)(0x20u | (next_random() & 0xffu));
        sys.ram[0x100u | (uint8_t)(initial_cpu.sp + 1u)] = 0x00u;
        sys.ram[0x100u | (uint8_t)(initial_cpu.sp + 2u)] = 0x90u;
        memcpy(initial_ram, sys.ram, GAM4980_RAM_SIZE);

        lcd_dirty = 0;
        if (s6502_firmware_hle_pixel_tail(
        ...
        if (reference_cycles != actual_cycles ||
            ...
            fprintf(stderr,
                "pixel-tail mismatch case=%lu x=%u mode=%u alias=%u "
                "cycles=%lu/%lu ram=%04lx dirty=%d/%d\n"
                "ref pc=%04x a=%02x x=%02x y=%02x sp=%02x p=%02x\n"
                "got pc=%04x a=%02x x=%02x y=%02x sp=%02x p=%02x\n",
                (unsigned long)case_id, sys.ram[0x2081u],
                sys.ram[0x2080u], s6502_page3[0xe5u],
    ...
    printf("firmware pixel-tail HLE: %lu exact-state cases passed\n",
