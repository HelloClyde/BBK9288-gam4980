...
def performance_globals(runtime):
    return "static c6502_u32 c6502_last_frame_tick;" + runtime.split(
        "static c6502_u32 c6502_last_frame_tick;", 1)[1].split(
        "static void bytes_set", 1)[0] + "\nstatic c6502_u32 native_clock(void){return 0;}\n"
...
class NativeRuntimeHelpersTest(unittest.TestCase):
    def test_query_box_contract_artwork_and_restore_match_rom(self):
        ...
        cc = shutil.which('gcc')
        if not cc: self.skipTest('host gcc is not installed')
        runtime = (ROOT/'src/c6502_native_runtime.c').read_text(encoding='utf-8')
        helper = '#include "c6502_native_query_assets.h"' + runtime.split(
            '#include "c6502_native_query_assets.h"',1)[1].split('/* E.BIN bank 9 $8195',1)[0]
        translate = 'static c6502_u8 translate_key' + runtime.split(
            'static c6502_u8 translate_key',1)[1].split('static void api_translate_message',1)[0]
        wrapper = 'static void query_box(c6502_u32 *r)' + runtime.split(
            'static void query_box(c6502_u32 *r)',1)[1].split('static void api_graphics',1)[0]

        def draw_hash(calls):
            data = bytearray()
            for name, args in calls:
                if name == 'clear':
                    for y in range(args[1],args[3]+1): data.extend((1,args[0],args[2],y,0))
                elif name in ('rect','fill'): data.extend((2,*args,int(name=='fill')))
                elif name == 'text': data.extend((3,*args[:2])); data.extend(bytes.fromhex(args[2])+b'\0')
                elif name == 'picture':
                    x,y,x1,y1,_,flag,pixels=args
                    self.assertEqual(flag,0)
                    width=x1-x+1; stride=(width+7)//8; pixels=bytes.fromhex(pixels)
                    for row in range(y1-y+1):
                        data.extend((4,x,y+row,width)); data.extend(pixels[row*stride:(row+1)*stride])
            h=2166136261
        ...
        for selection in (0,1):
            for keys in ((0x2f,),(0x2e,),(0x37,0x2f),(0x38,0x38,0x2f),
                         (0x35,0x2f),(0x39,0x2f),(2,3,0,1,0x2f),(0x15,),(0x26,)):
        ...
        for index,(text,selection,info_type,keys,alloc_ok) in enumerate(cases):
            ...
            checks.append('{ const unsigned char text[]={'+','.join(map(str,text+b'\0'))+'};'
                ...
                'polls=0;presents=0;h=2166136261u;key_count='+str(len(keys))+';alloc_ok='+str(int(alloc_ok))+';'
...
''' + translate + helper + wrapper + 'int main(void){unsigned r[15]={0};'+ '\n'.join(checks)+'''
...
'''
        with tempfile.TemporaryDirectory(prefix='c6502-query-rom-') as folder:
            path=Path(folder)/'test.c';exe=Path(folder)/'test.exe'
            path.write_text(source,encoding='utf-8')
            subprocess.run([cc,'-O2','-I',str(ROOT/'src'),str(path),'-o',str(exe)],check=True)
    ...
    def test_matrix_keys_are_edges_and_only_directions_repeat(self):
        ...
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        constants = "#define C6502_IO_BASE" + runtime.split("#define C6502_IO_BASE", 1)[1].split("extern const", 1)[0]
        keys = "static c6502_u8 c6502_key_previous[8];" + runtime.split(
            "static c6502_u8 c6502_key_previous[8];", 1)[1].split("static c6502_u32 c6502_last_frame_tick", 1)[0]
        helper = "static c6502_u8 poll_hardware_key" + runtime.split(
            "static c6502_u8 poll_hardware_key", 1)[1].split("static c6502_u32 native_clock", 1)[0]
        source = """
#include <assert.h>
...
"""
        with tempfile.TemporaryDirectory(prefix="c6502-matrix-") as folder:
            c = Path(folder) / "test.c"; exe = Path(folder) / "test.exe"
    ...
    def test_hardware_clock_keeps_seconds_and_rejects_invalid_samples(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helper = "static c6502_u32 native_clock(void)\n{" + runtime.split(
            "static c6502_u32 native_clock(void)\n{", 1)[1].split("static c6502_u8 native_window_key", 1)[0]
        source = """
#include <assert.h>
typedef unsigned char c6502_u8;
typedef unsigned int c6502_u32;
...
"""
    ...
    def test_poll_never_waits_without_queued_messages(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helpers = "static c6502_u8 native_window_key" + runtime.split(
            "static c6502_u8 native_window_key", 1)[1].split("static void native_update_timer", 1)[0]
...
"""
    ...
    def test_timer_api_matches_rom_and_elapsed_irq_count(self):
        ...
        if not cc: self.skipTest('host gcc is not installed')
        runtime = (ROOT/'src/c6502_native_runtime.c').read_text(encoding='utf-8')
        helpers = 'static void native_update_timer' + runtime.split(
            'static void native_update_timer',1)[1].split('static c6502_u8 translate_key',1)[0]
        ...
        for number in (0,1,5,200,242,255):
            ...
            cases.append(f'memset(ram,0,sizeof(ram));now=c6502_timer_clock=0;c6502_timer_remaining=256;native_timer_open({number});'
            ...
            got=invoke_firmware(memory,0x7b7d,0,b'').a
            cases.append(f'native_timer_open(17);assert(native_timer_number()=={got});native_timer_close();assert(!native_timer_number() && !(ram[0x226]&1));')
        ...
        for number in (0, 1, 5, 200, 255):
            for count in (0, 4, 199, 254, 255):
                ...
                cases.append('memset(ram,0,sizeof(ram));ram[0x226]=1;ram[0x227]=166;'
...
''' + globals_ + helpers + 'int main(void){' + '\n'.join(cases) + '''
...
'''
    ...
    def test_message_box_layout_timeout_and_background_match_rom(self):
        ...
        if not cc: self.skipTest('host gcc is not installed')
        runtime=(ROOT/'src/c6502_native_runtime.c').read_text(encoding='utf-8')
        timer='static void native_update_timer'+runtime.split('static void native_update_timer',1)[1].split('static c6502_u8 translate_key',1)[0]
        helper='static c6502_u8 message_box_layout'+runtime.split('static c6502_u8 message_box_layout',1)[1].split('static void query_box(',1)[0]
        globals_=performance_globals(runtime).replace('static c6502_u32 native_clock(void){return 0;}',
            'static unsigned now;static c6502_u32 native_clock(void){return now;}')
        # Odd/even GBK boundary, exact multiples, five-line clamp, empty input.
        texts=[b'', '百草地'.encode('gbk'),b'a'*15+'天地'.encode('gbk'),
               b'0123456789abcdef',b'0'*17,'天地玄黄宇宙洪荒日月盈昃辰宿列张'.encode('gbk'),
               b'A'*95, b'A'*256]
        checks=[]
        for text in texts:
            expected=trace_rom_message_box(text,5)
            calls=[(name,args) for name,args in expected['calls'] if name in ('text','rect','fill')]
            checks.append('{ const unsigned char s[]={'+','.join(map(str,text+b'\0'))+'};'
            ...
            for i,(name,args) in enumerate(calls):
                if name=='text':
                    ...
                    checks.append(f'expected[{i}][0]=0;expected[{i}][1]={args[0]};expected[{i}][2]={args[1]};')
                    for j,val in enumerate(vals+[0]):checks.append(f'expected_text[{i}][{j}]={val};')
                else:checks.append(f'expected[{i}][0]={1 if name=="rect" else 2};'+''.join(f'expected[{i}][{j+1}]={v};' for j,v in enumerate(args)))
            checks.append('ram[0x201a]=1;ram[0x2019]=7;ram[0x2018]=0;ram[0x226]=1;ram[0x227]=166;'
            ...
            if text and len(text)!=256:checks.append('assert(now-100>=12 && now-100<=15);')
            else:checks.append('assert(polls==0);')
            checks.append('}')
        source='''
#include <assert.h>
#include <string.h>
...
''' + timer + helper + 'int main(void){unsigned r[15]={0};'+ '\n'.join(checks)+'''
...
'''
    ...
    def test_file_catalog_and_random_access_survive_reopen(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helper = "#define C6502_FILE_RECORDS" + runtime.split("#define C6502_FILE_RECORDS",1)[1].split("static c6502_u32 signed_div",1)[0]
        names = sorted(set(re.findall(r'C6502_BRIDGE_\w+',helper)))
        defines = "enum {" + ",".join(names) + "};\n"
        defines += "\n".join("#define " + name + " 1" for name in sorted(set(re.findall(r'C6502_NEEDS_\w+',helper)))) + "\n"
        source = """
#include <assert.h>
#include <stdio.h>
#include <string.h>
...
"""
    ...
    def test_strcmp_returns_rom_int_not_stale_operand_pointer(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helper = "static void native_strcmp" + runtime.split("static void native_strcmp",1)[1].split("#define C6502_FILE_RECORDS",1)[0]
        checks = []
        words = [b"", b"a", b"b", b"z", b"abc", b"abcd", b"\x01", b"\x7f", b"\x80", b"\xff", "伏魔记0".encode("gbk"), "伏魔记1".encode("gbk")]
        for left in words:
            for right in words:
                ...
                checks.append("{unsigned char l[]={"+",".join(map(str,left+b"\0"))+"},b[]={"+",".join(map(str,right+b"\0"))+"};"
...
""" + helper + "int main(void){unsigned r[15]={0};" + "\n".join(checks) + "return 0;}"
    ...
    def test_native_output_centres_159_pixels_without_padding(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helpers = "static c6502_u16 lcd_address" + runtime.split("static c6502_u16 lcd_address",1)[1].split("static void rectangle(",1)[0]
        helpers += "void c6502_native_present" + runtime.split("void c6502_native_present",1)[1].split("static c6502_u8 map_key",1)[0]
        source = """
#include <assert.h>
#include <string.h>
typedef unsigned char c6502_u8;
...
""" + performance_globals(runtime) + helpers + """
int main(void){
...
"""
        with tempfile.TemporaryDirectory(prefix="c6502-output-") as folder:
            c=Path(folder)/"output.c"; exe=Path(folder)/"output.exe"
            c.write_text(source,encoding="utf-8")
            subprocess.run([cc,"-O2",str(c),"-o",str(exe)],check=True)
            subprocess.run([str(exe)],check=True)
    ...
    def test_completed_picture_frames_publish_without_input_polling(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helper = "static void api_graphics" + runtime.split(
            "static void api_graphics", 1)[1].split("static void native_strcmp", 1)[0]
...
'''
    ...
    def test_bitmap_adapters_match_original_firmware_bytes(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helpers = "static c6502_u16 lcd_address" + runtime.split("static c6502_u16 lcd_address",1)[1].split("static void line",1)[0]
        helpers += "static c6502_u8 screen_bit" + runtime.split("static c6502_u8 screen_bit",1)[1].split("/* The 9288 font renderer",1)[0]
        helpers += "static int picture_source_disjoint" + runtime.split("static int picture_source_disjoint",1)[1].split("static void integer_to_ascii",1)[0]
        checks = []
        shapes = [(0,0,159,95),(16,8,31,23),(1,3,8,10),(7,65,19,70),
                  (152,80,159,95),(0,65,7,65),(2,1,5,4),(31,23,16,8)]
        rng = random.Random(9288)
        for _ in range(32):
            x=rng.randrange(160); y=rng.randrange(96)
        ...
        for dummy in (False,True):
            for flag in (0,1):
                for x,y,x1,y1 in shapes:
                    ...
                    args = bytes([y,x1,y1,0,0x90]) + (b"\x00\x30" if dummy else b"") + bytes([flag])
                    invoke_firmware(mem,0x5b18 if dummy else 0x682d,x,args)
                    start,end = (0x3000,0x3780) if dummy else (0x401,0x1001)
                    expected=2166136261
                    for byte in mem.ram[start:end]: expected=((expected ^ byte)*16777619)&0xffffffff
                    call = "picture_dummy(r)" if dummy else f"native_picture({x},{y},{x1},{y1},0x9000,{flag})"
                    checks.append("{ unsigned char args[]={"+",".join(map(str,args))+"}; "
...
""" + helpers + "\nint main(void){unsigned r[15]={0};int errors=0;\n" + "\n".join(checks) + "\nreturn errors!=0;}\n"
        ...
        source = source.split("int main(void)",1)[0] + "int main(void){unsigned r[15]={0};int errors=0;for(unsigned i=0;i<16;++i)c6502_native_state.banks[i]=512;\n" + "\n".join(checks) + "\nreturn errors!=0;}\n"
        with tempfile.TemporaryDirectory(prefix="c6502-bitmap-abi-") as folder:
            c=Path(folder)/"bitmap.c"; exe=Path(folder)/"bitmap.exe"
            c.write_text(source,encoding="utf-8")
            subprocess.run([cc,"-O2",str(c),"-o",str(exe)],check=True)
            subprocess.run([str(exe)],check=True)
    ...
    def test_byte_division_and_shifts_match_rom_abi(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helpers = "static void return8" + runtime.split("static void return8", 1)[1].split("static void runtime_compare16", 1)[0]
        rom = (ROOT / "应用/数据/游戏/gam4980/E.BIN").read_bytes()[0xa8000:0xab000]
        checks = []
        cases = [(0xda09, "runtime_shift(r,8,0)", 8),
        ...
        for pc, call, bits in cases:
            for value in [0,1,7,0x7f,0x80,0xff,0x8000,0xffff]:
                for count in [0,1,2,7,8,9,15,16,31,255,256,0x1000]:
                    cpu = MPU(); cpu.memory[0xd000:] = rom
                    initial = [0x5a,0xa5,0x39,count & 255,count >> 8,0x79,0x41,0x31]
                    if bits == 16: initial[:2] = [value & 255,value >> 8]
                    cpu.memory[0x20:0x28] = initial
                    cpu.a = value & 255 if bits == 8 else 0xa5
                    cpu.x=0x47; cpu.y=0x79; cpu.p=0x71; cpu.pc=pc
                    cpu.sp=0xfd; cpu.memory[0x1fe:0x200]=[0xff,0x1f]
                    for _ in range(600):
                        if cpu.pc == 0x2000: break
                        cpu.step()
                    self.assertEqual(cpu.pc, 0x2000)
                    checks.append("{ unsigned char init[8]={" + ",".join(map(str,initial)) + "}; memcpy(ram+0x20,init,8); "
                        f"r[4]={value & 255 if bits == 8 else 0xa5};r[5]=0x47;r[6]=0x79;r[9]=0x71;{call}; "
                        f"assert(r[4]=={cpu.a} && r[5]=={cpu.x} && r[6]==0x79 && r[9]=={cpu.p}); "
                        + "".join(f"assert(ram[{0x20+i}]=={v});" for i,v in enumerate(cpu.memory[0x20:0x28])) + "}")
        source = """
#include <assert.h>
#include <string.h>
typedef unsigned char c6502_u8;
...
""" + helpers + "\nint main(void){unsigned r[15]={0};\n" + "\n".join(checks) + "\nreturn 0;}\n"
        with tempfile.TemporaryDirectory(prefix="c6502-byte-abi-") as folder:
            c=Path(folder)/"arithmetic.c"; exe=Path(folder)/"arithmetic.exe"
            c.write_text(source,encoding="utf-8")
            subprocess.run([cc,"-O2",str(c),"-o",str(exe)],check=True)
            subprocess.run([str(exe)],check=True)
    ...
    def test_text_wraps_at_a_series_width_and_erases_old_cell_pixels(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helper = "static const c6502_u8 *native_glyph" + runtime.split("static const c6502_u8 *native_glyph",1)[1].split("static int picture_source_disjoint",1)[0]
        source = """
#include <assert.h>
#include <string.h>
typedef unsigned char c6502_u8;
...
""" + performance_globals(runtime) + helper + """
int main(void){
...
"""
        with tempfile.TemporaryDirectory(prefix="c6502-text-test-") as folder:
            c=Path(folder)/"text.c"; exe=Path(folder)/"text.exe"
            c.write_text(source,encoding="utf-8")
            subprocess.run([cc,"-O2",str(c),"-o",str(exe)],check=True)
            subprocess.run([str(exe)],check=True)
    ...
    def test_compare16_matches_c6502_rom_flags_and_register_results(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        helper = runtime.split("static void runtime_compare16", 1)[1].split("static void *native_target", 1)[0]
        rom = (ROOT / "应用/数据/游戏/gam4980/E.BIN").read_bytes()
        ...
        for left,right in pairs:
            cpu = MPU(); cpu.memory[0xd340:0xd362] = code
            cpu.memory[0x20:0x22] = [left & 255,left >> 8]
            cpu.memory[0x23:0x25] = [right & 255,right >> 8]
            cpu.pc=0xd340; cpu.p=0x34; cpu.y=0x79
            for _ in range(40):
                if cpu.pc==0xd361: break
                cpu.step()
            self.assertEqual(cpu.pc,0xd361)
            nz = 0 if cpu.p & 2 else (128 if cpu.p & 128 else 1)
            checks.append(f"r[9]=0x34; r[6]=0x79; runtime_compare16(r,{left},{right}); "
...
static void runtime_compare16""" + helper + "\nint main(void) { unsigned r[15]={0};\n" + "\n".join(checks) + "\nreturn 0;}\n"
        with tempfile.TemporaryDirectory(prefix="c6502-compare-test-") as folder:
            c = Path(folder)/"compare.c"; exe=Path(folder)/"compare.exe"
            c.write_text(source,encoding="utf-8")
            subprocess.run([cc,"-O2",str(c),"-o",str(exe)],check=True)
            subprocess.run([str(exe)],check=True)
    ...
    def test_heap_reuses_freed_blocks_and_lcd_mapping_matches_reference(self):
        ...
        if not cc:
            self.skipTest("host gcc is not installed")
        runtime = (ROOT / "src/c6502_native_runtime.c").read_text(encoding="utf-8")
        heap = runtime.split("static void native_heap_init", 1)[1].split("static int lzss_expand", 1)[0]
...
static void native_heap_init""" + heap + "\nstatic c6502_u16 lcd_address" + lcd + """
int main(void) {
