...
import native_app_options as options
import compile_gam_native_9288 as driver
...
class NativeConverterTest(unittest.TestCase):
    ...
    def test_gam_inspection_and_rejection(self):
        ...
        self.assertEqual((result['title'], result['entry'], result['code_size']), ('魔塔', 0x5046, 96))
        game.write_bytes(b'bad')
        with self.assertRaises(ValueError): options.inspect_game(game)
        game = game_file(self.folder)
        data = bytearray(game.read_bytes()); struct.pack_into('<I', data, 0x42, 1000); game.write_bytes(data)
        with self.assertRaises(ValueError): options.inspect_game(game)
    ...
    def test_gbk_name_limit_and_literal(self):
        ...
        for name in ('', '\n', 'a'*16, '魔'*8, '游戏😀', 'a\0b'):
            with self.subTest(name=name), self.assertRaises(ValueError): options.encode_app_name(name)
    ...
    def test_default_icons_repack_identically(self):
        ...
        for name in ('ico1.bin','ico2.bin'):
            self.assertEqual((self.folder/name).read_bytes(), (ROOT/'assets/9288'/name).read_bytes())
    ...
    def test_custom_icon_aspect_frame_and_exact_preview(self):
        ...
        for name, image in (('ico1',big), ('ico2',small)):
            decoded = options.decode_icon(self.folder/'icons'/(name+'.bin'), image.width, image.height)
            self.assertEqual(bytes(v*85 for v in decoded), image.tobytes())

    def test_transparent_icon_rejected(self):
        source = self.folder/'empty.png'; Image.new('RGBA',(4,4),(0,0,0,0)).save(source)
        with self.assertRaises(ValueError): options.icon_images(source)
    ...
    def test_kf2_actual_metadata_and_default_compatibility(self):
        ...
        self.assertEqual(app[header[6]:header[6]+header[7]], (self.folder/'ico1.bin').read_bytes())
    ...
    def test_lock_rejects_concurrent_job_and_releases(self):
        ...
        with options.compiler_lock(lock):
            with self.assertRaises(RuntimeError):
                with options.compiler_lock(lock): pass
        with options.compiler_lock(lock): pass

    def args(self):
        return argparse.Namespace(game=game_file(self.folder), app_name='魔塔', icon=None,
            sdk=self.folder, toolchain=self.folder, output=self.folder/'原生.exe', work_dir=self.folder/'work')

    def test_failed_compilation_does_not_overwrite(self):
        args=self.args(); args.output.write_bytes(b'old-valid-exe')
        with patch.object(driver.subprocess,'run',side_effect=subprocess.CalledProcessError(1,['compiler'])):
            with self.assertRaises(subprocess.CalledProcessError): driver.convert(args)
        self.assertEqual(args.output.read_bytes(), b'old-valid-exe')

    def test_oversize_does_not_overwrite(self):
        args=self.args(); args.output.write_bytes(b'old-valid-exe')
        stage=args.work_dir/'native-result/program.exe';stage.parent.mkdir(parents=True)
        stage.write_bytes(b'X'*driver.MAX_KF2_BYTES)
        with patch.object(driver.subprocess,'run'):
            with self.assertRaises(ValueError): driver.convert(args)
        self.assertEqual(args.output.read_bytes(), b'old-valid-exe')

    def test_success_publishes_complete_report(self):
        args=self.args(); stage=args.work_dir/'native-result/program.exe';stage.parent.mkdir(parents=True)
        for suffix in ('.exe','.elf','.map'):stage.with_suffix(suffix).write_bytes(b'new-valid-artifact')
        report=args.work_dir/'c6502-s1c33-direct/report.json';report.parent.mkdir();report.write_text('{}')
        with patch.object(driver.subprocess,'run'): driver.convert(args)
    ...
    def test_atomic_copy_failure_leaves_destination(self):
        source=self.folder/'source';source.write_bytes(b'new');dest=self.folder/'target';dest.write_bytes(b'old')
        with patch.object(driver.shutil,'copyfile',side_effect=OSError('disk full')):
            with self.assertRaises(OSError): driver.publish_file(source,dest)
    ...
    def test_gui_command_preserves_unicode_and_spaces(self):
        config=gui.Conversion(self.folder/'测试 游戏.gam','魔塔',None,self.folder/'我的 游戏.exe',self.folder,self.folder)
    ...
    def test_cancel_before_spawn_does_not_start_compiler(self):
        worker=gui.ConversionWorker(queue.Queue()); worker.cancel()
        config=gui.Conversion(self.folder/'a.gam','魔塔',None,self.folder/'out.exe',self.folder,self.folder)
        with patch.object(gui.Conversion,'validate'), patch.object(gui.subprocess,'Popen') as popen, patch.object(gui,'ROOT',self.folder):
            worker.run(config)
    ...
    def test_cancel_running_owned_process(self):
        ...
        with patch.object(gui.Conversion,'validate'), patch.object(gui.Conversion,'command',return_value=command), patch.object(gui,'ROOT',self.folder):
            thread=threading.Thread(target=worker.run,args=(config,));thread.start()
            try:
                deadline=time.monotonic()+10
                ready=False
                while time.monotonic()<deadline:
                    kind,data=worker.events.get(timeout=10)
                    if kind=='line' and data.strip()=='ready':ready=True;break
                    if kind=='error':self.fail(data)
                self.assertTrue(ready)
            finally:
                worker.cancel();thread.join(10)
