...
DEFAULT_MAP = Path(r"D:\Downloads\BBK旧A系列C语言开发包\dwn\test.map")
...
def symbols_by_address(path: Path) -> dict[int, str]:
    ...
    for address, names in candidates.items():
        ...
        result[address] = min(
            names,
            key=lambda item: (
                0 if item.startswith("_Sys") else
                1 if item.startswith("__") else 2,
                len(item), item,
            ),
        ).lstrip("_")
    return result
...
def decode_record(
    game: bytes, record: tuple[int, ...]
) -> list[native.aotgen.InstructionIR]:
...
def canonicalize_records(
    game: bytes, records: list[tuple[int, ...]],
) -> list[tuple[int, ...]]:
    ...
    for record in records:
        ...
        if cursor != end:
            raise RuntimeError(
                f"overlapping block entry 0x{end:x} splits an instruction"
            )
...
def block_successors(
    ...
    next_pc = (record[1] + record[3]) & 0xFFFF
    if opcode == 0x20:
        return (next_offset,)
    if opcode in native.aotgen.BRANCH_FLAG_READS:
        ...
        return tuple(
            item for item in (physical, next_offset)
            if item is not None
        )
...
def analyze(game: bytes, map_path: Path) -> dict[str, object]:
    ...
    for name, value in re.findall(
        r"#define C6502_RT_([A-Z0-9_]+) 0x([0-9a-fA-F]{4})u", generated
    ):
    ...
    for record in records:
        ...
        for index in range(max(0, len(instructions) - 4)):
            ...
            if tuple(item.data[0] for item in window) != (
                0xA2, 0x86, 0xA2, 0x86, 0x20
            ):
                continue
            if (
                window[1].data[1] != 0x26 or
                window[3].data[1] != 0x27 or
            ...
            table_offset = native.game_far_table_offset(
                len(game), stats["code_size"], table,
            )
            if table_offset is not None:
                ...
                if (
                    bank >= 0xE0 and
                    GAME_VIRTUAL_BASE <= target < GAME_VIRTUAL_BASE + GAME_BANK_SIZE and
                    0 <= target_offset < stats["code_size"]
                ):
    ...
    owned_guest_instructions = sum(
        record_by_offset[item][4] for item in owned_blocks
    )
    semantic_calls = (
        sum(runtime_calls.values()) + sum(firmware_calls.values()) +
    ...
    call_transport_guest_instructions = (
        far_call_sites * 5 + sum(runtime_calls.values()) +
        sum(direct_firmware_calls.values())
    )
    ...
    report: dict[str, object] = {
        ...
        "remaining_opcodes": {
            ...
            for opcode, count in remaining_opcodes.most_common()
        },
        ...
        "unresolved_far_tables": {
            f"0x{key:04x}": value
            for key, value in unresolved_far_tables.most_common()
        },
        "unresolved_direct_targets": {
            f"0x{key:04x}": value
            for key, value in unresolved_direct_targets.most_common()
        },
    }
    return report
