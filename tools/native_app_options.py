...
from convert_9288_icon import decode_icon, encode_icon
...
def encode_app_name(name: str) -> bytes:
    ...
    if len(encoded) > MAX_NAME_BYTES:
        raise ValueError(f"名称为 {len(encoded)} 个 GBK 字节，最多 {MAX_NAME_BYTES} 字节（约 7 个汉字）。")
    return encoded
...
def default_app_name(stem: str) -> str:
    ...
    for char in stem.strip():
        try:
            if len((result + char).encode("gbk")) > MAX_NAME_BYTES: break
            if ord(char) >= 32: result += char
...
def inspect_game(path: Path) -> dict:
    if path.suffix.lower() != ".gam": raise ValueError("请选择 .gam 游戏文件。")
    size = path.stat().st_size
    with path.open("rb") as file: header = file.read(0x46)
    ...
    return dict(path=str(path.resolve()), size=size, code_size=code_size,
                resource_size=size-code_size, entry=entry,
...
def icon_images(source: Path | None = None) -> tuple[Image.Image, Image.Image]:
    ...
    if source is None:
        ...
        for stem, size in (("ico1",40),("ico2",16)):
            pixels = decode_icon(ROOT/"assets/9288"/(stem+".bin"),size,size)
            image = Image.new("L",(size,size));image.putdata([p*85 for p in pixels]);images.append(image)
        return tuple(images)
    ...
    if box is None: raise ValueError("图片完全透明，请选择有可见内容的图标。")
