...
import build_9288
from native_app_options import encode_app_name, default_app_name, c_name_definition, write_icons
...
def runtime_supported_bridges() -> tuple[str, ...]:
    ...
    return tuple(sorted(set(re.findall(
        r"\bcase\s+C6502_BRIDGE_([A-Za-z0-9_]+)\s*:", runtime
    ))))
...
def undefined_symbols(readelf: str, obj: Path) -> list[str]:
    text = subprocess.check_output(
        [readelf, "-s", str(obj)], text=True, errors="replace"
    )
    result = []
...
def verify_9288_abi(clang: str) -> None:
    ...
    result = subprocess.run(
        [clang, "--target=s1c33-none-elf", "-Os", "-ffreestanding",
         "-S", "-x", "c", "-", "-o", "-"],
        input=source, text=True, capture_output=True, check=True,
    )
    if not all(re.search(r"ld\.w\s+%r4,\s*%r" + str(reg) + r"\b",
                         result.stdout) for reg in (6, 9)):
        raise SystemExit(
    ...
    instructions = [line.strip() for line in result.stdout.splitlines()
                    if line.strip() and not line.lstrip().startswith((".", "#"))]
    for previous, following in zip(instructions, instructions[1:]):
        if re.match(r"(?:call|ret)\.d\b", previous) and "%sp" in following:
            raise SystemExit("S1C33 compiler has the unsafe SP-read delay-slot bug; "
...
def generate_bridges(symbols: list[str], output: Path) -> tuple[Path, Path]:
    ...
    if unsupported:
        raise ValueError(
            "native runtime has no explicit implementation for: "
            + ", ".join(unsupported)
        )

    header = output / "c6502_native_bridge_ids.h"
    assembly = output / "c6502_native_bridges.S"
    enum_lines = ["#ifndef C6502_NATIVE_BRIDGE_IDS_H", "#define C6502_NATIVE_BRIDGE_IDS_H", "enum {"]
    bridge_ids = {
        symbol: index for index, symbol in enumerate(SUPPORTED_BRIDGES, 1)
    }
    ...
    def save_registers(symbol: str, ident: int) -> list[str]:
        body = [f"    .globl {symbol}", f"    .type {symbol},@function", f"{symbol}:",
                "    sub %sp, 4", "    ld.w [%sp+0], %r3"]
    ...
    for symbol in symbols:
        ...
        if symbol in (
            ...
            lines.extend(["    popn %r3", "    ld.w %r3, [%sp+0]", "    add %sp, 4", "    cmp %r11, 0"])
            if symbol == "c6502_runtime_switch_comparison":
                lines.extend(["    jreq .Lbridge_return_%d" % ident, "    jp %r11", ".Lbridge_return_%d:" % ident, "    ret"])
            else:
                lines.extend(["    jreq .Lbridge_return_%d" % ident, "    call %r11", ".Lbridge_return_%d:" % ident, "    ret"])
        else:
            lines += reload_registers()
...
def main() -> None:
    ...
    parser.add_argument("--sdk", type=Path, default=Path(r"D:\Downloads\app_env_9288"))
    parser.add_argument("--toolchain", type=Path, default=ROOT / "build" / "llvm-s1c33-host")
    parser.add_argument("--output", type=Path, default=ROOT / "build" / "9288" / "FUMO-NATIVE.exe")
    parser.add_argument("--app-name")
    parser.add_argument("--icon", type=Path)
    parser.add_argument("--work-dir", type=Path, default=ROOT / "build" / "9288")
    ...
    run([
        sys.executable, str(ROOT / "tools" / "compile_c6502_s1c33_direct.py"),
        str(args.game.resolve()), "--clang", clang, "--objcopy", objcopy,
        "--output", str(direct), "--jobs", "4",
    ])
    ...
    if "c6502_runtime_banked_function_call" in external:
        raise SystemExit("Unresolved FAR call in native game: recover its target "
    ...
    flags = [
        "--target=s1c33-none-elf", "-Os", "-ffreestanding", "-fno-builtin",
        "-fno-jump-tables", "-fomit-frame-pointer", "-fdata-sections",
        "-ffunction-sections", "-fno-strict-aliasing", "-DDL_DOWN", "-D_RLS_",
        f"-DC6502_GAME_SIZE={args.game.stat().st_size}u",
        "-Wno-pointer-to-int-cast", "-Wno-int-to-pointer-cast",
        "-I", str(ROOT / "src" / "9288_compat"), "-I", str(sdk_include),
        "-I", str(ROOT / "src"), "-I", str(runtime_build),
        "-include", str(config),
    ]
    objects = [combined]
    for source in (
        ROOT / "src" / "c6502_native_9288_start.c",
        ROOT / "src" / "c6502_native_9288.c",
        ROOT / "src" / "c6502_native_runtime.c",
    ...
    run([
        clang, "--target=s1c33-none-elf", "-nostdlib",
        "-Wl,-T," + str(ROOT / "src" / "c6502_native_9288.ld"),
        "-Wl,-Map," + str(map_path), "-Wl,--gc-sections",
