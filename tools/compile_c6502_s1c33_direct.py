...
import compile_c6502_semantic_native as functions  # noqa: E402
import c6502_lzss  # noqa: E402
...
import export_c6502_boot_snapshot as boot_export  # noqa: E402
...
def direct_ram(address: int, write: bool = False) -> bool:
    if address < 0x100:
        special = (
            native.aotgen.PAGE0_SPECIAL_WRITE if write
            else native.aotgen.PAGE0_SPECIAL_READ
        )
...
def indexed_indirect_address(zp: int) -> list[str]:
    ...
    result.extend(["    call c6502_direct_read16_wrap", "    add %r12, %r6", "    ld.uh %r12, %r12"])
    return result
    ...
    if name in ("add16_preserve", "sub16_preserve"):
        ...
        if source == 0x28 and destination == 0x28:
            return imm(operation, "r10", value) + [
                "    ld.uh %r10, %r10", "    ld.w %r4, %r10", "    srl %r4, 8", "    ld.ub %r4, %r4",
            ]
    if name == "store16_imm" and values[0] == 0x28:
        value = values[1]
        return (
            imm("ld.w", "r10", value) + ["    ld.uh %r10, %r10"] +
            imm("ld.w", "r4", (value >> 8) & 0xff) + ["    ld.ub %r8, %r4"]
        )
    if name == "copy16" and values[1] == 0x28:
        source = values[0]
        if source == 0x28:
            return ["    ld.w %r4, %r10", "    srl %r4, 8", "    ld.ub %r4, %r4", "    ld.ub %r8, %r4"]
        result = load_memory(source, "r10")
        result.extend(load_memory((source + 1) & 0xff, "r12"))
        result.extend(["    ld.w %r4, %r12", "    sll %r12, 8", "    or %r10, %r12", "    ld.uh %r10, %r10", "    ld.ub %r8, %r4"])
        return result
...
def collect_semantic_outlines(
    game: bytes, bodies: dict[int, list[tuple[int, ...]]],
) -> dict[str, str]:
...
def complete_function_partition(
    game: bytes, records: list[tuple[int, ...]], entries: set[int],
    ...
    for offset in remaining:
        targets = tuple(
            target for target in frontend.block_successors(
                game, record_by_offset[offset]
            )
...
def compile_semantic_outlines(
    clang: str, objcopy: str, output: Path, outlines: dict[str, str],
) -> dict[str, object]:
...
def compile_resource_image(
    clang: str, output: Path, game: bytes,
) -> dict[str, object]:
...
def compile_boot_snapshot(
    clang: str, output: Path, game_path: Path, rom8: Path, rome: Path,
) -> dict[str, object]:
...
def indirect_target_working_set(game, records, emitted_entries, modules):
    ...
    for module in modules:
        if any("indirect_call" in key or "banked_function_call" in key
               for key in module["call_kinds"]):
...
def compile_dispatch_table(
    clang: str, output: Path, offsets: Iterable[int],
) -> dict[str, object]:
...
def immediate_argument(
    instructions: list[native.aotgen.InstructionIR], kind: str,
) -> int | None:
...
def native_call_transaction(
    game: bytes, code_size: int, records: list[tuple[int, ...]], start: int,
    predecessor_counts: Counter[int], entries: set[int],
    firmware_table: dict[int, str], direct_symbols: dict[int, str],
    ...
    while index < len(records) and len(arguments) < 4:
        ...
        index += 1
        if (
            index >= len(records) or records[index][2] != next_offset or
            predecessor_counts[next_offset] != 1
        ):
            return None
    ...
    if (
        cleanup_index >= len(records) or
        records[cleanup_index][2] != next_offset or
        predecessor_counts[next_offset] != 1
    ):
        return None
...
def emit_transaction_argument(
    game: bytes, record: tuple[int, ...], argument_index: int,
    kind: str, immediate_value: int | None, outlines: dict[str, str],
) -> list[str]:
    ...
    if kind == "int":
        ...
        result.extend([
            "    ld.ub %r13, [%r14]", "    ld.w %r4, %r13", "    sll %r13, 8",
            f"    or %{destination}, %r13", f"    ld.uh %{destination}, %{destination}",
            "    ld.ub %r4, %r4", "    ld.w %r6, 1",
        ])
        return result
    raise ValueError(kind)


def emit_rmw_helper(name: str, address: int, live_writes: int, indexed_x: bool = False) -> list[str]:
    result = imm("ld.w", "r12", address)
    if indexed_x:
...
def emit_instruction(ir: native.aotgen.InstructionIR) -> list[str]:
    ...
    if opcode in (0xAA, 0xA8, 0x8A, 0x98, 0xBA):
        destination, source = {
            0xAA: ("r5", "r4"), 0xA8: ("r6", "r4"),
            0x8A: ("r4", "r5"), 0x98: ("r4", "r6"),
            0xBA: ("r5", "r7"),
        }[opcode]
    ...
    if opcode in (0xC8, 0x88, 0xE8, 0xCA):
        register, operation = {
            0xC8: ("r6", "add"), 0x88: ("r6", "sub"),
            0xE8: ("r5", "add"), 0xCA: ("r5", "sub"),
        }[opcode]
    ...
    if opcode in (0x48, 0x08):
        ...
        result.extend(["    add %r12, %r7", f"    ld.b [%r12], %{source}", "    sub %r7, 1", "    ld.ub %r7, %r7"])
        return result
    ...
    rmw = {
        0x06: ("asl_m", False), 0x0E: ("asl_m", False), 0x1E: ("asl_m", True),
        0x26: ("rol_m", False), 0x2E: ("rol_m", False), 0x3E: ("rol_m", True),
        0x46: ("lsr_m", False), 0x4E: ("lsr_m", False),
        0x66: ("ror_m", False), 0x6E: ("ror_m", False),
        0xC6: ("dec_m", False), 0xCE: ("dec_m", False), 0xDE: ("dec_m", True),
        0xE6: ("inc_m", False), 0xEE: ("inc_m", False),
    }
    if opcode in rmw:
...
def condition_lines(
    opcode: int, unique: str, target: str, fallthrough: str,
    fallthrough_is_next: bool,
) -> list[str]:
    take_when_set = opcode in (0x30, 0x70, 0xB0, 0xF0)
    flag = {0x10: FLAG_N, 0x30: FLAG_N, 0x50: FLAG_V, 0x70: FLAG_V,
            0x90: FLAG_C, 0xB0: FLAG_C, 0xD0: FLAG_Z, 0xF0: FLAG_Z}[opcode]
    result: list[str] = []
    if flag == FLAG_Z:
    ...
    result.append(
        f"    jp .Lfallthrough_{unique}" if fallthrough_is_next
        else f"    jp {fallthrough}"
    )
...
def emit_transfer(target: int | None, local: set[int], entries: set[int], next_record: int | None) -> list[str]:
...
def emit_function(
    game: bytes, code_size: int, entry: int, records: list[tuple[int, ...]],
    entries: set[int], runtime_symbols: dict[int, str],
    firmware_table: dict[int, str], direct_symbols: dict[int, str],
    outlines: dict[str, str], aliases: set[int],
    ...
    while record_index < len(records):
        transaction = native_call_transaction(
            game, code_size, records, record_index, predecessor_counts,
            entries, firmware_table, direct_symbols,
        )
        if transaction is not None:
            ...
            for argument_index, argument in enumerate(transaction["arguments"]):
                ...
                lines.extend(emit_transaction_argument(
                    game, argument_record,
                ...
                lines.extend([
                    f"    sub %r10, {width}", "    ld.uh %r10, %r10",
                    "    ld.w %r12, %r14", "    add %r12, %r10",
                    f"    ld.b [%r12], %r{arg_reg}",
                ])
                if width == 2:
                    lines.extend([
                        f"    ld.w %r13, %r{arg_reg}", "    srl %r13, 8",
                        "    add %r12, 1", "    ld.b [%r12], %r13",
                    ])
        ...
        body_end = len(instructions) - (
            5 if table is not None else 1 if terminal_is_control else 0
        )
        ...
        if opcode in native.aotgen.BRANCH_FLAG_READS:
            ...
            lines.extend(condition_lines(
                opcode, f"{record[2]:05x}", transfer_label(target, local, entries),
                transfer_label(next_offset, local, entries), next_offset == next_record,
            ))
            continue
        if opcode in (0x4C, 0x80):
            target_pc = (terminal.data[1] | terminal.data[2] << 8) if opcode == 0x4C else native.aotgen.branch_target(terminal.pc, terminal.data[1])
            target = frontend.physical_target(record, target_pc)
            if target is not None:
                ...
                api = runtime_symbols[target_pc]
                if api == "switch_comparison" and native.game_switch_targets(game, record[2]+record[3]-3) is None:
...
def compile_bank(
    clang: str, objcopy: str, output: Path, bank: int, game: bytes,
    code_size: int, entries: set[int], records: list[tuple[int, list[tuple[int, ...]]]],
    runtime: dict[int, str], firmware: dict[int, str], direct: dict[int, str],
    outlines: dict[str, str], aliases: dict[int, set[int]],
) -> dict[str, object]:
    section = f".text.c6502_direct_{bank:02d}"
    lines = ["/* Generated direct S1C33; no guest opcode dispatcher. */", f'    .section "{section}","ax",@progbits']
    calls: Counter[str] = Counter()
    for entry, body in records:
        body_lines, body_calls = emit_function(
            game, code_size, entry, body, entries, runtime, firmware, direct,
            outlines, aliases.get(entry, {entry}),
        )
    ...
    far_targets = sorted({int(match.group(1), 16) for line in lines
                          if (match := re.fullmatch(
                              r"    call \.Lc6502_far_([0-9a-f]+)", line))})
    for target in far_targets:
        ...
        lines.extend([
            ...
            "    ld.w %r0, %r11", "    pushn %r0",
            f"    call {functions.function_name(target)}",
            "    popn %r0", "    ld.w %r11, %r0",
            "    call c6502_native_set_codebank", "    popn %r0", "    ret",
        ])
    ...
    s_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    subprocess.run([clang, "--target=s1c33-none-elf", "-c", str(s_path), "-o", str(o_path)], check=True)
    subprocess.run([objcopy, "-O", "binary", f"--only-section={section}", str(o_path), str(b_path)], check=True)
    return {
        "bank": bank,
        ...
        "native_call_sites": sum(
            value for key, value in calls.items()
            if not key.startswith("lifted_")
        ),
        "lifted_runtime_calls": sum(
            value for key, value in calls.items()
            if key.startswith("lifted_runtime:")
        ),
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "build" / "9288" / "c6502-s1c33-direct")
    ...
    for pattern in ("c6502_direct_bank_*.S", "c6502_direct_bank_*.o", "c6502_direct_bank_*.bin"):
        for path in args.output.glob(pattern):
            path.unlink()
    for pattern in ("c6502_semantic_outlines.S", "c6502_semantic_outlines.o", "c6502_semantic_outlines.bin"):
        for path in args.output.glob(pattern):
            path.unlink()
    ...
    outline_module = compile_semantic_outlines(
        args.clang, args.objcopy, args.output, outlines,
    )
    resource_image = compile_resource_image(args.clang, args.output, game)
    boot_snapshot = compile_boot_snapshot(
        args.clang, args.output, args.game, args.rom8, args.rome,
    )
    work = [(bank, sorted(items)) for bank, items in sorted(grouped.items())]
    with ThreadPoolExecutor(max_workers=max(1, args.jobs)) as pool:
        futures = [pool.submit(compile_bank, args.clang, args.objcopy, args.output, bank, game, stats["code_size"], entries, items, runtime, firmware, direct, outlines, aliases) for bank, items in work]
        modules = [future.result() for future in futures]
    modules.sort(key=lambda item: int(item["bank"]))
    emitted_entry_pattern = re.compile(
        r"^\s*\.globl\s+c6502_game_fn_([0-9a-f]+)\s*$", re.MULTILINE,
    )
    ...
    for module in modules:
        ...
        emitted_entries.update(
            ...
            for match in emitted_entry_pattern.finditer(
                source.read_text(encoding="utf-8")
            )
        )
    indirect_entries, indirect_scope = indirect_target_working_set(
        game, recovered, emitted_entries, modules,
    )
    dispatch_table = compile_dispatch_table(
        args.clang, args.output, indirect_entries,
    )
    report: dict[str, object] = {
        ...
        "preserved_game_data_bytes": (
            len(game) - sum(record[3] for record in recovered)
        ),
        ...
        "native_code_bytes": (
            sum(int(item["native_bytes"]) for item in modules) +
            int(outline_module["native_bytes"])
        ),
        ...
        "register_abi": {"A": "R4", "X": "R5", "Y": "R6", "SP": "R7", "lazy_nz": "R8", "P": "R9", "software_stack": "R10", "RAM": "R14"},
        ...
        "semantic_outline_sites": sum(
            int(item["call_kinds"].get("semantic_outlined", 0))
            for item in modules
        ),
    ...
    report["native_bytes_per_guest_instruction_x1000"] = int(report["native_code_bytes"]) * 1000 // instructions
    report["uncompiled_recovered_code_bytes"] = (
        int(report["recovered_code_bytes"]) -
        int(report["compiled_guest_bytes"])
    )
    report["native_plus_preserved_data_bytes"] = (
        int(report["native_code_bytes"]) +
        int(report["preserved_game_data_bytes"])
    )
